module RemoteExecutionHelper
  ...
  def job_report_template
    template = ReportTemplate.where(name: Setting['remote_execution_job_invocation_template']).first
    return template if template && template.template_inputs.where(name: 'job_id').exists?
  end
